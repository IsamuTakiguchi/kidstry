...
10問おわったところで、まちがえた問題があれば
**「▶ ○もん やる ／ けっかを みる」** を選べます。

- 復習では**選択肢の並び順だけ**が変わります（問題と正解はそのまま）。
  場所で覚えてしまうのを防ぐためです。
- **★の数と「おうちのかた」ページの正答率は、最初の10問の1回目の解答だけ**で決まります。
  復習しても変わりません（まちがえても復習すれば★3、という抜け道を作らないため）。
- 全問1回で正解したときは、この画面は出ずにそのまま けっか へ進みます。
- 記録・シール・メダルは**10問おわった時点で保存**されるので、
  復習の途中でホームに戻っても、その回の記録は消えません。
...
- 10問おわると、まちがえた数どおりの復習の案内が出る
- 「もういちど」で復習ラウンドに入り、最後までいくと結果に復習の回数が出る
- 「けっかを みる」ならそのまま結果へ進む
- **復習しても★と記録は変わらない／復習の途中で抜けても記録が消えない**
- 🔒のあそびはシールを集めるまで**開けない**
